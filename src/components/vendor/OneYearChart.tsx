...
import { addMonths, eachMonthOfInterval, isSameMonth } from 'date-fns';
...
const OneYearChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const from = addMonths(new Date(), -12);
        ...
        months.forEach((month) => {
          xAxis.push(String(month).slice(4, 7));

          reqsOnMarket.push(
            res1.resolvedRequests.filter((req) =>
              isSameMonth(new Date(month), new Date(req._creationTime)),
            ).length,
          );
  ...
  return <canvas ref={chartRef} width="400" height="200" id="OneYearChart" />;
};
